...
void convertSmoothing(cv::Mat &input, cv::Mat &processed);
void getEdge(cv::Mat &input, cv::Mat &processed);
...
void convertSmoothing(cv::Mat &input, cv::Mat &processed)
{
  ...
  //4. convert color to gray
  cv::GaussianBlur(input, processed, cv::Size(101, 101), 10);
...
#if FLAG // use built-in function
  cv::Mat temp;
  convertColorToGray(input, temp);
  cv::Sobel(temp, processed, -1,1,0);
